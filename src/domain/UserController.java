...
public class UserController {
	...
	/*
	 * Displays menu for user, gets input and depending on the input decides
	...
	protected void addActionListener(HomeScreen home) {
		home.addButtonActionListener1(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				String userAction = ((javax.swing.JButton) evt.getSource())
						.getName();
	...
	/*
	 * Redirects you to either moderator option site, administrator option site
	 * or tells you that you dont have the access
	 */
	...
	private void friendList() {
		...
		fl.setVisible(true);
		fl.buttonActionListener(new ActionListener() {
	...
	private void getFriends() {
		...
		try {
			/*
			 * Calls a stored procedure that will return a list of friends as a
			 * 2 dimensional array
			 */
			friendArray = connect.executeQuery("CALL Create_Friendlist('"
					+ uName + "');");
			/*
			 * If the user does not have any friends, stored procedure will not
			 * have returned anything and the 2 dimensional array will still
			 * equal null, and we will stop the method
			 */
			if (friendArray == null) {
				return; // TODO
			}
			/*
			 * Passes the 2 dimensional array to intialize the friends arraylist
			 * and binarytree
			 */
	...
	private void parseToArraylist(Object[][] friendArray) {
		/*
		 * Each friend in the 2 dimensional array will be saved in the friends
		 * arraylist and binarytree
		 */
	...
	private void addFriend(String newFriend) {
		try {
			connect.executeUpdate("INSERT INTO userRelations VALUES('"
					+ user.getUserName() + "','" + newFriend + "')");
	...
	private boolean isNameAvailable(String userName) {
		...
		try {
			/*
			 * If the username exists in the database, it will save the username
			 * in nameAvailability, if not the value will stay null
			 */
			nameAvailability = connect
					.executeQuery("SELECT userName FROM users WHERE userName = '"
							+ userName + "'");

			/*
			 * If the nameAvailabilty is empty there is no such name in the
			 * database and it will return true, else it will return false
			 */
	...
	private void getUsernameAndSeeDest(String prompt) {
		...
		usernameScreen.addButtonListeners(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				if (action.equals("1")) {
					...
					if (friends.contains(name)) {
						...
						usernameScreen.setVisible(false);
						getUsernameAndSeeDest(name
								+ " is not a friend of you, try again");
					}
				}
			}
		});
		...
		usernameScreen.addButtonListeners(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				if (action.equals("1")) {
					 ...
					 * the users is already friends with that person and it will
					 * let the user know
					 */
					if (friends.contains(name)) {
						usernameScreen.setVisible(false);
						getUsernameAndAdd(name
								+ " is already your friend, try another name");
						/*
						 * If the the name that the user tries to add is not
						 * available there is a user with that name and that
						 * user will be added as a friend
						 */
						...
						/*
						 * The last possibilty is that the username entered does
						 * not exist and it will let the user know
						 */
					} else {
						usernameScreen.setVisible(false);
